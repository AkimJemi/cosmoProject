...
=======
	public ArrayList<Sub> subSelectAll(Connection conn, Properties pagingValues)
			throws SQLException {
		ArrayList<Sub> SubMixed = new ArrayList<>();

		if (pagingValues.getProperty("search") != null) {
			String search = pagingValues.getProperty("search");
			pstmt = conn.prepareStatement(
					"select w.no, w.water_ind, e.ele_basic, e.ele_ind from ele e, water w where w.no = e.no having e.ele_basic like ? or e.ele_ind like ? or w.water_ind like ? or w.no like ?");
>>>>>>> be475a442771350ff55f0f6b9e89b56684a9a29c
...
<<<<<<< HEAD
			...
			pstmt.setInt(6, subPaging.getLimit() * (subPaging.getCurrentPage()-1));
		} else {
			pstmt = conn.prepareStatement(
			...
			pstmt.setInt(2, subPaging.getLimit() * (subPaging.getCurrentPage()-1));
=======
		} else {
			pstmt = conn.prepareStatement(
					"select w.no, w.water_ind, e.ele_basic, e.ele_ind from ele e, water w where w.no = e.no");
>>>>>>> be475a442771350ff55f0f6b9e89b56684a9a29c
		}
		try {
